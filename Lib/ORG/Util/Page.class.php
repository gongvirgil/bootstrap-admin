...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('header'=>'条记录','prev'=>'&laquo;','next'=>'&raquo;','first'=>'第一页','last'=>'最后一页','theme'=>'<ul class="pagination"><li class="disabled"><a>%totalRow% %header%</a></li> <li class="disabled"><a>%nowPage%/%totalPage% 页</a></li> %upPage% %first%  %prePage%  %linkPage%  %nextPage% %end% %downPage% </ul>');
    // 默认分页变量名
    protected $varPage;
        ...
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                ...
                if($this->totalPages != 1){
                    $linkPage .= "&nbsp;<li class='active disabled'><a href='#'>".$page."</a></li>";
                }
            }
        }
        $pageStr     =   str_replace(
